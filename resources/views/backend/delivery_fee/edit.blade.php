...
@section('title', __('Update Delivery Fee'))

@section('content')
    <x-forms.patch :action="route('admin.delivery_fee.update', $deliveryFee)"  :enctype="'multipart/form-data'">
        <x-backend.card>
            <x-slot name="header">
                @lang('Update Delivery Fee ')
            </x-slot>
            ...
            <x-slot name="body">
                <div>
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            {{ html()->select('city_id')
                                ...
                                ->options([
                                    $deliveryFee->city->id => $deliveryFee->city->name
                                    ])
                                ->value($deliveryFee->city->id)
                                ->id('city_id')
                                ->required()
                                ->class('form-control')}}
                        </div><!--col-->
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            {{ html()->select('district_id')
                                ...
                                ->options([
                                    $deliveryFee->district->id => $deliveryFee->district->name
                                    ])
                                ->value($deliveryFee->district->id)
                                ->id('district_id')
                                ->required()
                                ->class('form-control')}}
                        </div><!--col-->
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            {{ html()->select('ward_id')
                                ...
                                ->options([
                                    $deliveryFee->ward->id => $deliveryFee->ward->name
                                    ])
                                ->value($deliveryFee->ward->id)
                                ->id('ward_id')
                                ->required()
                                ->class('form-control')}}
                        </div><!--col-->
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            <input name="fee" id="fee" class="form-control" type="text" value="{{ old('fee') ? str_replace('.','',old('fee')) : $deliveryFee->fee }}" />
                        </div>
                    </div><!--form-group-->
            ...
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">@lang('Update Delivery Fee ')</button>
            </x-slot>
        </x-backend.card>
    </x-forms.patch>
@endsection
