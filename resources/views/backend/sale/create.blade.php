...
@section('content')
    <x-forms.post :action="route('admin.sale.store')" :enctype="'multipart/form-data'">
        <x-backend.card>
            <x-slot name="header">
                @lang('Create Sale')
            </x-slot>
                    ...
                    <div class="form-group row">
                        <label for="sale_amount" class="col-md-2 col-form-label">@lang('Discount Price')</label>
                            ...
                            <div class="col-md-10 row no-margin-left">
                                <input type="number" id="sale_amount" name="sale_amount" class="form-control w-25"  value="{{ old('sale_amount') ?? 0 }}" />
                                <div class="type-sale_amount ml-2 pt-2">
                                    <input type="radio" name="type" id="percent" value="0" {{ !old('type') ? 'checked' : '' }}>
                                    <label for="percent">%</label>
                                    <input type="radio" name="type" id="money" value="1" {{ old('type') ? 'checked' : '' }}>
            ...
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">@lang('Create Sale')</button>
            </x-slot>
        </x-backend.card>
    </x-forms.post>
@endsection
