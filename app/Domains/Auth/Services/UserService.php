...
class UserService extends BaseService
{
     ...
     * @param  User  $user
     */
    public function __construct(User $user, CustomerDetailRepository $customerDetailRepository, ReceiveInfoRepository $receiveInfoRepository)
    {
        $this->model = $user;
    ...
    public function registerUser(array $data = []): User
    {
        ...
        try {
            ...
            if (! $user->isMasterAdmin()) {
                if (isset($data['receive-info'])) 
                {
                    $this->receiveInfoRepository->create($user);
                }
                $this->customerDetailRepository->createUser($data, $user->id);
            }
        } catch (Exception $e) {
            DB::rollBack();
